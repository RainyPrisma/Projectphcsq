...
// ดึงประเภทสินค้าทั้งหมดจากตาราง product
$sql_categories = "SELECT DISTINCT nameType 
                   FROM product 
                   WHERE nameType IS NOT NULL 
                   AND nameType NOT REGEXP '^[0-9]+$' 
                   ORDER BY nameType ASC";
$categories_result = $conn->query($sql_categories);
$categories = [];
if ($categories_result) {
    while ($row = $categories_result->fetch_assoc()) {
        $categories[] = $row['nameType'];
    }
}
...
// Add search filter if provided (ค้นหาเฉพาะ name)
if (!empty($search_term)) {
    $sql .= " AND p.name LIKE '%" . $conn->real_escape_string($search_term) . "%'";
}
